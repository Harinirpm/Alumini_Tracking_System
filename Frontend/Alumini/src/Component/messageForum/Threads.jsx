...
function Threads({ selectedCategory, open }) {
    ...
    const fetchThreads = async () => {
      try {
        const response = await axios.get(
          `https://alumini-tracking-system.onrender.com/threads/${user.id}`
        );
  ...
  const handleClick = async (threadId) => {
    try {
      const response = await axios.post(
        `https://alumini-tracking-system.onrender.com/threads/like/${threadId}/${user.id}`
      );
  ...
  const filteredThreads = threads.filter(
    (thread) =>
      ...
      selectedCategory === "" ||
      thread.thread_domain === selectedCategory
  );
        ...
        filteredThreads.map((item, index) => (
          ...
          >
            <Box sx={{ display: "flex", gap: "20px", alignItems: "center" }}>
              <img
                src={
                  item?.created_by_details.profile_image_path
                    ? `https://alumini-tracking-system.onrender.com/${item?.created_by_details.profile_image_path.replace(
                        /\\/g,
                        "/"
                      )}`
                    : Img
              ...
              <Typography
                sx={{ color: "#6D6C80", fontSize: "12px", fontWeight: 600 }}
              >
                {" "}
              ...
              >
                {item.liked ? (
                  <FavoriteOutlinedIcon
                    sx={{ color: "#FF0505", cursor: "pointer" }}
                  />
                ) : (
